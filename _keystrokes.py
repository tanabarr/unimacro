__version__ = "$Rev: 508 $ on $Date: 2013-08-10 11:43:54 +0200 (za, 10 aug 2013) $ by $Author: quintijn $"
...
"""
import time, string, os, sys, inivars, types
import natlink
import nsformat
...
class ThisGrammar(ancestor):
    ...
    name = 'keystrokes'
    stdkeysrule = "<stdkeys> = <contextmenu> | ([<modifiers>] ((<repkey>[<count> %s]) | <norepkey>));" # times to be filled in!
    allkeysrule = """<allkeys> = <lastkey> |
        ((<click>|<stdkeys>)+ [<lastkey>]);"""
...
""" 
    ...
    def constructGramSpec(self, inInit=0):
        ...
        if self.doKeystrokesExtended:
            startrule = "<startrule> exported = [<before>+] <allkeys>;"
            ...
            startSpec = "# keystrokes simple\n" + \
                            startrule + '\n' + stdkeysrule
        #print 'startSpec: %s\n\n'% startSpec
        self.gramSpec = startSpec + '\n' +  self.lastpartofgramspec
        if not inInit:
...
##        print 'keystrokes, new mode: ', mode
        ...
        if mode == 'inactive':
            ...
            print 'activate keystrokes (no special mode)'
    ...
    ##        print 'fillInstantVariables for %s'% self
            ...
            for m in self.exclusiveModes:
                if m not in self.modes:
                    print 'warning, exclusive mode "%s" not in defined modes: %s'% (m, self.modes.keys())

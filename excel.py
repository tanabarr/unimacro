...
from actionsexcel import ExcelAction
#connect to program excel:
...
class ThisGrammar(ancestor):
    ...
    gramSpec = """
<col> exported = column ({character}|{customcolumn}|back) [<excelcolumncommand>];
<date> exported = date ({n1-31} [{month} [{year}]])|
                        {datespecial};
...
"""+numberGram
    ...
    def refreshExcelSettings(self, res):
        ...
        """
        self.curCol, self.curRow = self.excel.getCurrentPosition()
        if res <= 1:
            # only position
            return
    ...
    #        prevRow = self.popFromListIfDifferent(self.prevRows, self.curRow)
    #        if prevRow:                
    ...
    def gotoCol(self, col):
        ...
        col = col.strip()
        if col == self.curCol:
            print 'col already selected'
            return
        range = col + self.curRow
